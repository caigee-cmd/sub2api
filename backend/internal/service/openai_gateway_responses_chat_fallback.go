...
func (s *OpenAIGatewayService) forwardResponsesViaRawChatCompletions(
	...
	// Keep the final outbound tier (after policy filter/force) for usage-time reconciliation.
	// The final value is decided by resolvedOpenAIUpstreamServiceTier (upstream echo wins).
	// A policy filter that removes the field therefore leaves this nil, so the request is
	// no longer billed as Fast.
...
func (s *OpenAIGatewayService) bufferChatCompletionsAsResponses(
	...
	responsesResp := apicompat.ChatCompletionsResponseToResponses(ccResp, originalModel, customTools, functionTools, toolSearch, namespaceTools, strings.HasPrefix(strings.ToLower(upstreamModel), "qwen"))
