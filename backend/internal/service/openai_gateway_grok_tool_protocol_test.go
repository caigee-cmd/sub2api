...
func TestSanitizeGrokResponsesTools_SimplifiesNonObjectUnionSchema(t *testing.T) {
	t.Parallel()
	...
	mixed := gjson.GetBytes(sanitized, `tools.#(name=="mcp__codex_app__automation_update")`)
	...
	require.Equal(t, "mcp__codex_app__automation_update", gjson.GetBytes(sanitized, "tool_choice.name").String())
}
...
func TestSanitizeGrokResponsesTools_UnionSchemaBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "object-only union kept", tool: `{"type":"function","name":"a","parameters":{"oneOf":[{"type":"object"}]}}`},
		{name: "nested object-only union kept", tool: `{"type":"function","name":"a","parameters":{"anyOf":[{"type":"object","properties":{"p":{"anyOf":[{"type":"object"},{"type":"object"}]}}}]}}`},
		{name: "legal object kept", tool: `{"type":"function","name":"a","parameters":{"type":"object","properties":{}}}`},
		{name: "no parameters kept", tool: `{"type":"function","name":"a"}`},
		{name: "null parameters kept", tool: `{"type":"function","name":"a","parameters":null}`},
		{name: "mixed union simplified", tool: `{"type":"function","name":"a","parameters":{"oneOf":[{"type":"object"},{"type":"string"}]}}`, simplified: true},
		{name: "unproven branch simplified", tool: `{"type":"function","name":"a","parameters":{"oneOf":[{"properties":{}}]}}`, simplified: true},
		{name: "empty union kept", tool: `{"type":"function","name":"a","parameters":{"anyOf":[]}}`},
		{name: "array branch simplified", tool: `{"type":"function","name":"a","parameters":{"oneOf":[{"type":"object"},{"type":"array"}]}}`, simplified: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.simplified {
				...
				require.True(t, gjson.GetBytes(sanitized, "tools.0.parameters.additionalProperties").Bool())
				require.False(t, gjson.GetBytes(sanitized, "tools.0.parameters.oneOf").Exists())
				require.False(t, gjson.GetBytes(sanitized, "tools.0.parameters.anyOf").Exists())
			}
		})
	}
}

func TestSanitizeGrokResponsesTools_SimplifiesMixedUnionAndKeepsSiblings(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	require.True(t, json.Valid(sanitized))
	require.Equal(t, 2, int(gjson.GetBytes(sanitized, "tools.#").Int()))
	require.Equal(t, "object", gjson.GetBytes(sanitized, `tools.#(name=="bad").parameters.type`).String())
	require.Equal(t, "good", gjson.GetBytes(sanitized, `tools.#(name=="good").name`).String())
	require.Equal(t, "bad", gjson.GetBytes(sanitized, "tool_choice.name").String())
